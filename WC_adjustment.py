...
comp = fr"WorkersComp_newrates.csv"
df = pd.read_csv(comp, index_col=False)

path = fr"P:\PACS\Finance\Budgets\{folder}\Received\New folder\*.xlsx"
...
for file in glob(path):
    ...
    if not df.loc[df['Facility'] == file_name, 'WC_Wages'].empty:
        new_amount = df.loc[df['Facility'] == file_name, 'WC_Wages'].values[0]
        print(f"Match for {file_name}: {new_amount}")
        time.sleep(1)
        wb = xw.Book(file, update_links=False)
        time.sleep(9)
        main_page = wb.sheets("FORECAST WORKSHEET")
        insurance_line = main_page.range("C525").value
        monthly_drop = main_page.range("C524").value
        main_page.range("C525").value = new_amount
        main_page.range("C524").value = '% of Gross Wages'
        time.sleep(3)
